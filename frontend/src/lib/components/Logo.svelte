  ...
  $effect(() => {
    if (rotate) {
      window.addEventListener("scroll", () => {
        if (logo) {
          logo.style.transform = `rotate(${(window.scrollY * speed) % 360}deg)`;
        }
      });
    }
  });
